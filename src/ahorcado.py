...
def enmascarar_palabra(palabra, letras_probadas):
    ...
    '''
    list_caracteres=[] #Inicializar lista vacía
    for letra in palabra:
        if  letra in letras_probadas: #Elemento en lista
            list_caracteres.append #Añadir la letra en la lista
        else: 
            list_caracteres.append("_")
    return  ' '.join(list_caracteres) # Devuelve "a b c"
...
def pedir_letra(letras_probadas):
    ...
    while letra in letras_probadas: #Siempre pregunta hasta que acierte. Dif while-if.
        letra = input("Esa ya la dijiste, di otra: ").lower()
    return letra 
...
def comprobar_palabra_completa(palabra_secreta, letras_probadas):
    ...
    '''
    for letras_probadas in palabra_secreta:
        if letras_probadas  palabra_secreta:
            return True
        else:
            return False
